...
public class WatchfulService extends Service {
    ...
    public void buildReorderAndLaunch(boolean isToggled) {
        if (isToggled) {
            ...
            public void run() {
                try {
                    ...
                    if (failCount >= 2) {
                        ...
                        //WatchfulService.this.buildTasks(); // commented out kind of exception handling
                    }
                }
            }
        };
    ...
    protected final Runnable scanApps = new Runnable(){
        public void run(){
            ...
            buildTasks();
            //handler.postDelayed(this, LOOP_SECONDS * 1000);  // Removed infinite msg sending
        }
    };
...
//        Tools.HangarLog("taskList.size(): " + taskList.size() + " pageList.size(): " + pageList.size() + " realmaxbuttons: " + numOfApps + " maxbuttons: " + maxButtons + " moreAppsPage: " + moreAppsPage);
        ...
        if (!notificationBg.equals(Settings.NOTIFICATION_BG_DEFAULT_VALUE)) {
            customNotifView.setInt(R.id.notifContainer, "setBackgroundColor", 0);
        }
